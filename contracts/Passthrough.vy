...
def __init__(_reward_receivers: DynArray[address, 10], _guards: DynArray[address, 7], _distributors: DynArray[address, 10]):
    ...
    # Get admins from factory and set as immutables
    OWNERSHIP_AGENT = staticcall Factory(FACTORY).ownership_admin()
    PARAMETER_AGENT = staticcall Factory(FACTORY).parameter_admin()
    EMERGENCY_AGENT = staticcall Factory(FACTORY).emergency_admin()

    # Set up non-removable guards array with the agents
    self.non_removable_guards[0] = OWNERSHIP_AGENT
    self.non_removable_guards[1] = PARAMETER_AGENT
    self.non_removable_guards[2] = EMERGENCY_AGENT

    # Add factory admins and first guard as guards
